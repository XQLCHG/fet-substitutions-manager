...
def import_teachers(file: UploadFile = File(...), db: Session = Depends(get_db), _=Depends(require_admin)):
    ...
    for row_no, values in _rows(ws):
        try:
            ...
            profile = None
            if employee_no:
                profile = db.query(InvigilationTeacherProfile).filter(
            ...
            for header, role_code in ROLE_HEADER_MAP.items():
                ...
                default = None
                if is_new:
                    default = role_code in {"PRIMARY", "SECONDARY"}
                ...
                if not q:
                    q = InvigilationTeacherRoleQualification(
    ...
    if "不可用时间" in wb.sheetnames:
        ...
        for row_no, values in _rows(uw):
            try:
                employee_no = _text(values.get("工号"))
                name = _text(values.get("姓名"))
                query = db.query(InvigilationTeacherProfile)
                profile = None
                if employee_no:
                    profile = query.filter(InvigilationTeacherProfile.employee_no == employee_no).first()
                if not profile and name:
                    profile = query.filter(InvigilationTeacherProfile.professor_name == name).first()
                if not profile:
                    raise ValueError("找不到对应人员，请填写正确工号或姓名")
                day = _date(values.get("日期"))
                slot_code = _text(values.get("时段代码"))
                row = db.query(InvigilationTeacherUnavailability).filter(
                ...
                if slot_code is None:
                    existing = row.filter(InvigilationTeacherUnavailability.slot_code.is_(None)).first()
                else:
                    existing = row.filter(InvigilationTeacherUnavailability.slot_code == slot_code).first()
                if not existing:
...
def _location_code(location_type: str, path: str) -> str:
    digest = sha1(path.encode("utf-8")).hexdigest()[:14]
    return f"AUTO-{location_type}-{digest}"
...
def _get_or_create_location(db: Session, name: Optional[str], location_type: str,
    ...
    if parent is None:
        query = query.filter(InvigilationLocation.parent_id.is_(None))
    else:
        query = query.filter(InvigilationLocation.parent_id == parent.id)
    row = query.first()
    if not row:
...
def import_exams(
    ...
    roles = {r.code: r for r in db.query(InvigilationDutyRole).all()}
    created_sessions = created_exam_rooms = updated_exam_rooms = 0
    ...
    for row_no, values in _rows(ws):
        try:
            ...
            for loc_name, loc_type in names:
                ...
                location_by_type[loc_type] = parent

            room = location_by_type.get("ROOM")
            if not room:
            ...
            primary_count = _int(values.get("甲监人数"), roles["PRIMARY"].default_required)
            secondary_count = _int(values.get("乙监人数"), roles["SECONDARY"].default_required)
            _upsert_requirement(db, session.id, room.id, roles["PRIMARY"], primary_count)
            _upsert_requirement(db, session.id, room.id, roles["SECONDARY"], secondary_count)
            requirement_changes += 2

            roving_count = _int(values.get("流动监考人数"), None)
            floor = location_by_type.get("FLOOR")
            if roving_count is not None:
                if not floor:
                    raise ValueError("填写了流动监考人数时必须提供楼层")
                _upsert_requirement(db, session.id, floor.id, roles["ROVING"], roving_count)
                requirement_changes += 1

            inspector_count = _int(values.get("巡考人数"), None)
            area = location_by_type.get("AREA")
            if inspector_count is not None:
                if not area:
                    raise ValueError("填写了巡考人数时必须提供考区")
                _upsert_requirement(db, session.id, area.id, roles["INSPECTOR"], inspector_count)
                requirement_changes += 1
            db.flush()
        except Exception as exc:
            errors.append({"row": row_no, "message": str(exc)})
